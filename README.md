...
Code 속성은 클래스 파일에서 가장 중요한 속성이다(316p)</br>
자바 프로그램의 정보를 코드와 메타데이터(부가정보)로 나눈다면,</br>
클래스 파일 전체는 '코드를 설명하는 Code 속성'과</br>
'메타데이터를 설명하는 나머지 데이터 항목'으로 구분할 수 있다.</br>

#### 바이트코드 명령어(일부 살펴보기)

1. aload_0: 0번째 변수 슬롯에 담긴 참조 타입 지역 변수를 피연산자 스택의 맨 위로 읽어들이라는 뜻이다.(316p)</br>
2. invokespecial: stack 맨 위의 참조 타입 데이터가 가리키는 객체를</br>
   메서드의 수신자(receiver)로 사용하여 해당 객체의 인스턴스 생성자나</br>
   private 메서드 또는 상위 클래스의 메서드를 호출한다.</br>

- 데이터 교환과 메서드 호출 같은 작업이 모두 스택(피연산자 스택)을 기반으로 이루어진다.</br>
  --> 이를 통해 자바 가상 머신이 스택 기반 아키텍처라고 추측할 수 있다고 한다.</br>

<hr>
자바에서 this란,</br>
모든 인스턴스 메서드에서는 this 키워드를 통해 메서드가 속한 객체에 접근할 수 있다.</br>
javac(java compiler)가 소스 코드를(*.java) 컴파일할 때, this 키워드를 이용하는 코드를 발견하면</br>
일반적인 메서드 매개 변수를 통해 접근하는 코드로 변환한다.</br>
--> JVM에서는 인스턴스 메서드가 호출될 때, 해당 인스턴스를 가리키는 참조 값(this)이</br>
로컬 변수 0번 슬롯에 자동으로 전달된다.</br>
JVM은 메서드 호출시 인자들을(매개변수에 전달된 값) 로컬 변수 슬롯(local variable slots)에 할당한다.</br>
ex. 인스턴스 메서드 또는 인스턴스 생성자 실행시 해당 인스턴스를 가리키는 참조 값(this)이 자동으로 전달되며,</br>
이 값이 로컬 변수 슬롯 0번(slot 0)에 저장된다.
